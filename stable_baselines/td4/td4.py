...
from stable_baselines.td4.policies import TD4Policy


class TD4(OffPolicyRLModel):
    """
    Twin Delayed DDPG (TD4)
    Addressing Function Approximation Error in Actor-Critic Methods.

    :param policy: (TDPolicy or str) The policy model to use (MlpPolicy, CnnPolicy, LnMlpPolicy, ...)
    ...
    :param random_exploration: (float) Probability of taking a random action (as in an epsilon-greedy strategy)
        This is not needed for TD4 normally but can help exploring when using HER + TD4
    ...
    :param full_tensorboard_log: (bool) enable additional logging when using tensorboard
        Note: this has no effect on TD4 logging for now
    ...
    def __init__(self, policy, env, gamma=0.99, learning_rate=3e-4, buffer_size=50000,
                 ...
                 random_exploration=0.0, verbose=0, n_support=64, kappa=1, risk_factor=0,model_type="QR", tensorboard_log=None,
                 _init_setup_model=True, policy_kwargs=None,
                 full_tensorboard_log=False, seed=None, n_cpu_tf_sess=None):

        super(TD4, self).__init__(policy=policy, env=env, replay_buffer=None, verbose=verbose,
                                  policy_base=TD4Policy, requires_vec_env=False, policy_kwargs=policy_kwargs,
            ...
            with self.graph.as_default():
                ...
                with tf.variable_scope("model", reuse=False):
                    ...
                    if self.model_type == "QR":
                        self.qrtau = tf.reshape(tf.range(0.5 / self.n_support, 1, 1 / self.n_support),
                                           [1, self.n_support])
                        qf1, qf2 = self.policy_tf.make_critics(self.processed_obs_ph, self.actions_ph,n_support=self.n_support)
                        # Q value when following the current policy
                        qf1_pi, _ = self.policy_tf.make_critics(self.processed_obs_ph,
                                                                policy_out, reuse=True,n_support=self.n_support)
                    else :
                        self.qrtau = tf.random_uniform([tf.shape(self.processed_obs_ph)[0], self.n_support], minval=1e-5, maxval=1.0)
                        qf1, qf2 = self.policy_tf.make_critics(self.processed_obs_ph, self.actions_ph,
                                        model_type=self.model_type,iqn_tau=self.qrtau,n_support=self.n_support)
                        # Q value when following the current policy
                        qf1_pi, _ = self.policy_tf.make_critics(self.processed_obs_ph,
                                        policy_out, model_type=self.model_type,iqn_tau=self.qrtau,reuse=True,n_support=self.n_support)
                    self.qf1_pi = qf1_pi
                    tf.summary.histogram('qf1', qf1_pi[0])
                    #tf.summary.histogram('qf2', qf2[0])
                ...
                with tf.variable_scope("target", reuse=False):
                    ...
                    if self.model_type == "QR":
                        qf1_target, qf2_target = self.target_policy_tf.make_critics(self.processed_next_obs_ph,
                                                                                noisy_target_action,n_support=self.n_support)
                    else :
                        target_qrtau = tf.sort(
                            tf.random_uniform([self.batch_size, self.n_support], minval=1e-5, maxval=1.0), axis=1)
                        qf1_target, qf2_target = self.target_policy_tf.make_critics(self.processed_next_obs_ph,
                                noisy_target_action,model_type=self.model_type,iqn_tau=target_qrtau,n_support=self.n_support)
                    #tf.summary.histogram('qf1',qf1_target[0])
                    #tf.summary.histogram('qf2', qf2_target[0])

                with tf.variable_scope("loss", reuse=False):
                    # Take the min of the two target Q-Values (clipped Double-Q Learning)

                    quantile_weight = 1.0 - self.risk_factor * \
                                           (2.0 * self.qrtau - 1.0)
                    #min_qf_target = tf.cond(
                    #    tf.reduce_mean(qf1_target)<tf.reduce_mean(qf2_target),lambda: qf1_target,lambda: qf2_target)
                    min_qf_target = tf.minimum(qf1_target, qf2_target)
                    # Targets for Q value regression
                    q_backup = tf.stop_gradient(
                        self.rewards_ph +
                        self.gamma * (1.0 - self.terminals_ph) * min_qf_target
                    )

                    # Compute Q-Function loss
                    qrtau = tf.tile(tf.expand_dims(self.qrtau, axis=2), [1, 1, self.n_support])
                    #qrtau = tf.tile(tf.expand_dims(self.qrtau, axis=2), [1, 1, self.n_support])
                    ...
                    #Huber_loss = tf.compat.v1.losses.huber_loss(logit_valid_tile, theta_loss_tile,
                    #                                  reduction=tf.losses.Reduction.NONE,delta=self.kappa)/self.kappa
                    bellman_errors = logit_valid_tile - theta_loss_tile
                    Logcosh = bellman_errors + tf.math.softplus(-2. * bellman_errors) - tf.log(2.)
                    Loss = tf.abs(qrtau - tf.stop_gradient(tf.to_float(bellman_errors < 0))) * Logcosh
                    qf1_loss = tf.reduce_mean(tf.reduce_sum(tf.reduce_mean(Loss, axis = 1), axis = 1))

                    theta_loss_tile = tf.tile(tf.expand_dims(qf2, axis=2), [1, 1, self.n_support])
                    #Huber_loss = tf.compat.v1.losses.huber_loss(logit_valid_tile, theta_loss_tile,
                    #                                  reduction=tf.losses.Reduction.NONE, delta=self.kappa)/self.kappa
                    bellman_errors = logit_valid_tile - theta_loss_tile
                    Logcosh = bellman_errors + tf.math.softplus(-2.*bellman_errors) - tf.log(2.)
                    Loss = tf.abs(qrtau - tf.stop_gradient(tf.to_float(bellman_errors < 0))) * Logcosh
                    qf2_loss = tf.reduce_mean(tf.reduce_sum(tf.reduce_mean(Loss, axis = 1), axis = 1))
                    ...
                    self.policy_loss = policy_loss = -tf.reduce_mean(tf.reduce_mean(qf1_pi*quantile_weight))
                    #self.policy_loss = policy_loss = -tf.reduce_mean(qf1_pi * quantile_weight)
                    #ap_loss = -tf.reduce_mean(tf.reduce_mean(qf1_pi * quantile_weight))
                    #print(ap_loss)
    ...
    def learn(self, total_timesteps, callback=None,
              log_interval=4, tb_log_name="TD4", reset_num_timesteps=True, replay_wrapper=None):
        ...
        tb_log_name = tb_log_name + '_' + self.model_type + '_' + str(self.risk_factor)
    ...
    def action_probability(self, observation, state=None, mask=None, actions=None, logp=False):
        ...
        if actions is not None:
            raise ValueError("Error: TD4 does not have action probabilities.")

        # here there are no action probabilities, as DDPG does not use a probability distribution
        warnings.warn("Warning: action probability is meaningless for TD4. Returning None")
        return None
